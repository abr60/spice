...
```bash
omarchy plugin remove theme-scheduler
```
...
```bash
ln -sfn "$PWD" ~/.config/omarchy/plugins/theme-scheduler
omarchy plugin enable theme-scheduler --section right
```

## Commands

```bash
omarchy-shell theme-scheduler status
omarchy-shell theme-scheduler enable
omarchy-shell theme-scheduler disable
omarchy-shell theme-scheduler applyNow
```
