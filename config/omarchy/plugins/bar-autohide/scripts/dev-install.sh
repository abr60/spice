...
filtered = [
    ...
    if not (isinstance(entry, dict) and entry.get("id") in {"autohide", "bar-autohide"})
]
enabled = any(
    isinstance(entry, dict) and entry.get("id") == "bar-autohide"
...
if len(filtered) != len(plugins) or not enabled:
    ...
    if not enabled:
        filtered.append({"id": "bar-autohide"})
    ...
    try:
        ...
        os.unlink(temporary)
        raise
    print(f"Enabled bar-autohide; backup: {backup}")
else:
    print("bar-autohide is already enabled")
PY
